...
outFileName = "NonFlatEffCorrectedSameFidVol_Z"+str(ZMax)+"_"+str(YMax)+"_"+str(YMin)+"_"+str(XMax)+"_"+str(XMin)+"60.root"
print outFileName , " ------------------------------------- "
...
for entry in myTree:
     ...
     decision = efficiencyFunct(distance)
     if np.random.random_sample() > decision:
         continue
     ...
     if len(recoKESkim):
         hInteractingKE.Fill( recoKESkim[len(recoKESkim)-1], 1./decision )
